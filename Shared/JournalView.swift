...
struct JournalView: View {
    ...
    private var pageTitle = ["Ghost hunting for dummies", "Equipment", "Locating ghosts", "Determining animosity level", "Methods of extraction", "Determining the correct method for extraction"]
        ...
        Image("background-journal").resizable()
            ...
            .overlay(
                VStack(alignment: .leading) {
                    Text(pageTitle[page - 1]).font(Font.custom("Mousedrawn", size: 24)).padding(.bottom, 0.5)
                    Text(pageContent[page - 1]).lineSpacing(8)
            Spacer()
                    Spacer()
